...
//Define Staionary mode credentials for local router
const char* ssid     = "EXP1-ssid"; // Change this to your network
const char* password = "EXP1-password"; // Change this to your network
...
WiFi.mode (WIFI_STA); // Define mode to use Stationary
WiFi.begin(ssid,password); // Tell Wif to join using provided credentials
Serial.println("\nConnecting");
      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            client.println("<body><h1>Using Station Mode (Existing Network) </h1>");
